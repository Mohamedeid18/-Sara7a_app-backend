...
export const socialLogin = async (req, res) => {
    ...
    const {email, email_verified,given_name, family_name, picture} = await verifyGoogle({idToken});
    if(!email_verified) {
    ...
    if(user) {
        ...
        if(user.provider === ProviderEnum.GOOGLE) {
            const token = await getNewLoginCredentials(user);
            successResponse({res, message: 'Social login successful', data: {token}});
        } 
    } else {
        // User does not exist, create new user
        const newUser = await create({ model: UserModel, data: [{ email, firstName: given_name, lastName: family_name, provider: ProviderEnum.GOOGLE, profileImage: picture }] });
        const token = await getNewLoginCredentials(newUser);
        successResponse({res, message: 'User created and logged in successfully', data: {token}});
    }
};